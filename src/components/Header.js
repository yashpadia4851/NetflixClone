...
const Header = () => {
  ...
  return (
    <div className='absolute w-[1519px] px-8 py-2 z-10 flex justify-between bg-gradient-to-b from-black'>
    ...
    <div className='flex'>  
      {ShowGptSearch && 
       <select className='p-2 bg-gray-700 m-5 text-white rounded-lg' onChange={handleLanguageChange}>
        ...
        <img className='w-20 rounded-full' src={user?.photoURL} alt="User Icon" />
        <button onClick={handleSignout} className='mx-6 text-white font-bold'>(Sign Out)</button>
      </div>}
    </div>
  );
}
